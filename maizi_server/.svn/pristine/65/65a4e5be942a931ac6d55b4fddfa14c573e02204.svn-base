...
def excel_export(request):
    ...
    sheet.write(0, 8, u'应付金额')
    sheet.write(0, 9, u'优惠')
    sheet.write(0, 10, u'优惠后金额')
    sheet.write(0, 11, u'付费方式')
    sheet.write(0, 12, u'付款单号')
    sheet.write(0, 13, u'销售人员')
    sheet.write(0, 14, u'成单时间')
    sheet.write(0, 15, u'技术老师')
    sheet.write(0, 16, u'教务老师')
    sheet.write(0, 17, u'助学金')
    ...
    for i in range(1, len(student_list)+1):
        ...
        sheet.write(i, 9, student_list[i-1].get('reduction'))
        sheet.write(i, 10, student_list[i-1].get('pay_real'))
        sheet.write(i, 11, student_list[i-1].get('pay_type'))
        sheet.write(i, 12, student_list[i-1].get('pay_order_num'))
        sheet.write(i, 13, student_list[i-1].get('sailer'))
        sheet.write(i, 14, student_list[i-1].get('sail_at'), dateFormat)
        sheet.write(i, 15, student_list[i-1].get('teacher_name'))
        sheet.write(i, 16, student_list[i-1].get('assistant_name'))
        sheet.write(i, 17, student_list[i-1].get('reward_name'))
        filename = 'student_info.xls'
...
def assistant_excel_export(request):
    '''
    教务面板的数据导出
    :param request:
    :return:
    ...
    info = student_assistant_query(request)
    student_list = info['student_list']
    xls = xlwt.Workbook(encoding='utf-8')
    sheet = xls.add_sheet('sheet1')
    dateFormat = xlwt.XFStyle()
    ...
    sheet.write(0, 6, u'专业')
    sheet.write(0, 7, u'就业类型')
    sheet.write(0, 8, u'销售人员')
    sheet.write(0, 9, u'成单时间')
    sheet.write(0, 10, u'技术老师')
    sheet.write(0, 11, u'教务老师')
    sheet.write(0, 12, u'助学金')
    sheet.write(0, 13, u'开学时间')
    sheet.write(0, 14, u'毕业时间')
    sheet.write(0, 15, u'阶段')
    sheet.write(0, 16, u'状态')
    sheet.write(0, 17, u'就业公司')
    ...
    for i in range(1, len(student_list)+1):
        ...
        sheet.write(i, 6, student_list[i-1].get('career_name'))
        sheet.write(i, 7, student_list[i-1].get('career_job_name'))
        sheet.write(i, 8, student_list[i-1].get('sailer'))
        sheet.write(i, 9, student_list[i-1].get('sail_at'), dateFormat)
        sheet.write(i, 10, student_list[i-1].get('teacher_name'))
        sheet.write(i, 11, student_list[i-1].get('assistant_name'))
        sheet.write(i, 12, student_list[i-1].get('reward_name'))
        sheet.write(i, 13, student_list[i - 1].get('start_at'),dateFormat)
        sheet.write(i, 14, student_list[i - 1].get('graduate_at'), dateFormat)
        sheet.write(i, 15, student_list[i - 1].get('stage_name'))
        sheet.write(i, 16, student_list[i - 1].get('state_name'))
        sheet.write(i, 17, student_list[i - 1].get('company'))
        filename = 'assistant_student_info.xls'
        xls.save(os.path.join(settings.UPLOAD_DOCUMENT_DIRS, filename))
