...
def wiki_index(request):
    ...
    return render(request, 'mz_wiki/wiki.html', data)


# wiki分类详情页
def wiki_course_type(request, short_name):
    """
    wiki分类详情页
    :param request:
    :param short_name: wiki_type的short_name
    :return:
    """

    data = wiki_public_data()

    courses = db.api.get_wiki_courses_by_type_short_name(
        short_name)
    if courses.is_error():
        course_list = []
        log.warn(
            'wiki_course_type error: get courses failed. '
            'wiki_type short_name: {0}'.format(short_name))
    else:
        course_list = mark_course_list_first_item_short_name(courses.result())

    current_course_type = dict()
    for _type in data['type_list']:
        if _type['short_name'] == short_name:
            current_course_type = _type

    data.update(course_list=course_list, type_short_name=short_name,
                current_course_type=current_course_type)

    return render(request, 'mz_wiki/wikiList.html', data)
...
def wiki_item_detail(request, course_short_name, item_short_name):
    ...
    return render(request, 'mz_wiki/wikiArticle.html', data)
