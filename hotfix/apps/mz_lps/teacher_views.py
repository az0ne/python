...
def class_manage(request, class_id):
    ...
    if request.user not in class_.teachers.all():
...
def student_detail(request, class_id, user_id):
    ...
    for stage in stages:
        ...
        # 获取阶段中的课程
        courses = Course.objects.filter(stages_m = stage).order_by("index", "id")
...
def set_course_project_score(request):
    ...
    #stage_id = request.POST.get("stage_id") or 0
    try:
    ...
    try:
        ...
        # 更新课程得分表
        stuusertask_list = StuStatusUserTask.objects.filter(student_id=user.id, relate_id=course_id)
        for usertask in stuusertask_list:
            if usertask.status != 1:
                usertask.finish_date = datetime.now().strftime("%Y-%m-%d")
                usertask.status = 1
                usertask.save()
        ...
        # 加入学员动态：老师给项目制作打分
        if score:
            params = dict(user=user, type=3, obj_id=course.id)
        ...
        for stage in course.stages_m.all():
            stage_id = stage.id
            alert_msg = "老师已给你<a href='"+ \
                        "http://www.maiziedu.com/lps2/learning/plan/"+str(Stage.objects.get(pk=stage_id).career_course.id)+"/?stage_id="+ \
                        str(stage_id)+"'>"+str(course.name)+"</a>课程的项目制作打了分，<a href='"+ \
                        "http://www.maiziedu.com/lps2/learning/detail/"+str(Stage.objects.get(pk=stage_id).career_course.id)+"/?stage_id="+ \
                        str(stage_id)+"'>赶快去看看吧</a>！"
...
def add_mission(request):
    ...
    try:
        ...
        #### 给对应学员推送消息 开始 ####
        # 获取该职业课程老师班级下的所有学员
        class_list = Class.objects.filter(teachers=request.user, career_course=stage.career_course).values_list("id")
        ...
        for student in student_list:
            alert_msg = "老师在你的职业课程 - "+str(stage.career_course.name)+"添加了额外任务，<a href='"+\
                        "http://www.maiziedu.com/lps2/learning/plan/"+str(stage.career_course.id)+\
...
def teacher_save_remark(request):
    try:
        ...
        remark = cgi.escape(remark)
        rebuild_count = get_rebuild_count(user, course_id)
        project = Project.objects.get(examine_type=5, relation_type=2, relation_id=course_id)
        project_record = ProjectRecord.objects.get(project=project, student=user, rebuild_count=rebuild_count)
        project_record.remark = remark
        project_record.save()
    except Exception as e:
        return HttpResponse(json.dumps("no", ''),content_type="application/json")
    return HttpResponse(json.dumps(["ok", remark]),content_type="application/json")
