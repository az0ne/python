...
$(function(){
    ...
    function show_error_info(data){
        if(data.account){  //手机号错误
            $("#telphonetips").html(data.account);
        } else if(data.mobile){
            $("#telphonetips").html(data.mobile);
        } else if(data.mobile_f){  //手机号错误, 后端让人郁闷的各式各样的Form设计
            $("#telphonetips").html(data.mobile_f);
        } else if(data.captcha){  //验证码错误
            $("#yzmtips").html(data.captcha);
        } else if(data.mobile_code_f){  //手机验证码错误
            $("#phoneyzmtips").html(data.mobile_code_f);
    ...
    function send_random_code(){
        $.ajax({
            type: "POST",
            url: "/user/password/send_sms_code/",
            data: {
                account: $('#telphone').val(),
                code: $('#captcha').val(),
    ...
    // 发送注册信息
    function send_find_password_info(){
        $.ajax({
            type: "POST",
            url:"/user/password/reset_password/",
            data:{
                account: $('#telphone').val(),
                mobile_f: $('#telphone').val(),
                mobile_code_f: $('#phoneyzm').val(),
            ...
            success: function(data){
                if(data.status == 'success'){ //弹出修改密码成功的提示
                    $(".formSuccess").addClass("show");
                    $(".applyForm .bg").addClass("show");
                } else {
                    show_error_info(data);
                    if(data.status == 'failure'){
                        $(".formFail").addClass("show"); //弹出密码失败
                        $(".applyForm .bg").addClass("show");
                    }
                }
            },
    ...
    function find_password(){
        ...
        send_find_password_info();
    };


    //$(".formSuccess").addClass("show");
    //$(".formFail").addClass("show");
    //$(".applyForm .bg").addClass("show");

    // 密码修改之后(成功或者失败), 弹出窗口, 绑定窗口的关闭按钮
	$(".formReviewed .closed").bind("touchend",function(){
        $(this).parent().removeClass("show");
        $(".applyForm .bg").removeClass("show");
    });

    // 密码修改成功之后, 弹出提示框, 绑定该提示框的"前去登陆"
    $("#login").bind("touchend", function(){
