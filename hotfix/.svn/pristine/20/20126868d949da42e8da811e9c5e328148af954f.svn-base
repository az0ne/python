...
var init = function(){
    ...
    onPraise();
    function onPraise(){
        var praise = $('.praise-btn'),off = true;
    ...
    $.ajax({
        ...
        data: {'object_type': 'ARTICLE', 'object_id': articleId},
        success: loadSuccess
    });
    ...
    function onReplay(obj){
        ...
        if(str != '' && ueSmall.getContentTxt() == '说点什么吧...'){
            ...
            setTimeout(function(){
                oBtn.find('span').css('left','-100px').text('');
            }, 2000);
            ueSmall.setContent('');
            ...
            $.ajax({
                ...
                data: {'object_type': 'ARTICLE', 'object_id': articleId, 'parent_id': parent_id, 'comment': str},
    ...
    $('.submit_reviews').on({'click':function(){
        ...
        if(str != '' && ue.getContentTxt() == '说点什么吧...'){
            ...
            $.ajax({
                type: 'POST',
                url: '/common/ajax/add/discuss',
                data: {'object_type': 'ARTICLE', 'object_id': articleId, 'parent_id': 0, 'comment': str} ,
    ...
    function loadSuccess(data){
        var html = '', parent_id, oList = $('.reviews_lists');
        ...
        if(data.length > 0){
            for(var i =0; i < data.length; ++i){
                ...
                if(data[i].parent_id == 0){
                    ...
                    html += '<div class="col_l"><a href="/u/' + data[i].user_id + '" target="_blank"><img src="'+ MEDIAURL + data[i].head +'" alt="'+ data[i].nick_name +'"></a></div>';
                    html += '<div class="col_r" style="width: 710px;">';
                    ...
                    for(var j =0; j < data.length; ++j){
                        if(data[j].parent_id == parent_id){
                            ...
                            html += '<div class="col_l"><a href="/u/' + data[j].user_id + '" target="_blank"><img src="'+ MEDIAURL + data[j].head +'" alt="'+ data[j].nick_name +'"></a></div>';
