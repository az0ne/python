...
class LoginForm(forms.Form):
    ...
    登录Form
    '''
    error_messages = {"invaild": "该账号格式不正确", "disabled": "该账号已被禁用"}
...
class EmailRegisterForm(forms.Form):
    ...
    error_messages = {"duplicate_email": "该账号已被注册",
                      "length_error": "8-20位，区分大小写，不支持空格",}
    ...
    password = forms.CharField(widget=forms.PasswordInput(attrs={"class": "form-control", "placeholder": "请输入密码"}),
                               ...
                               error_messages = {
                                   "required":"请输入密码",
                                   "min_length":"请输入8-20位密码，区分大小写",
                                   "max_length":"请输入8-20位密码，区分大小写"})
...
class MobileRegisterForm(forms.Form):
    ...
    error_messages = {
        ...
        # "invaild": "注册账号需为手机格式",
        "invaild": "手机号码格式不正确，请重新输入",
        "nonmatch_mobile_code":"手机验证码输入错误，请重试",
        "overdue_mobile_code":"手机验证码已过期，请重试",
        "length_error": "8-20位，区分大小写，不支持空格",
    }
    mobile = forms.CharField(widget=forms.TextInput(attrs={"class": "form-control form-control-phone fl", "placeholder": "请输入你的手机号"}),
                             max_length=11, error_messages = {"required":"请输入你的手机号",
                                                              "min_length":"手机号码格式不正确，请重新输入",
                                                              "max_length":"手机号码格式不正确，请重新输入",})
    mobile_code = forms.CharField(max_length=6, min_length=6, widget=forms.TextInput(attrs={"class": "form-control", "placeholder": "请输入短信验证码"}),
                                  error_messages = {
                                      "required":"请输入手机验证码",
                                        "min_length":"校验码是6位数字，请重新输入",
                                        "max_length":"校验码是6位数字，请重新输入"})
    password_m = forms.CharField(widget=forms.PasswordInput(attrs={"class": "form-control", "placeholder": "请输入密码"}),
                               min_length=8, max_length=20,
                               error_messages = {
                                   "required":"请输入密码",
                                   "min_length":"8-20位，区分大小写，不支持空格",
                                   "max_length":"8-20位，区分大小写，不支持空格"})

    # captcha_m = CaptchaField(required=False,widget=CaptchaTextInput(
    #     attrs={"class": "form-control form-control-captcha fl","placeholder": "请输入验证码"}),
    #                        error_messages = {"required":"请输入验证码","invalid":"验证码错误"})

    def clean_password_m(self):
        password = self.cleaned_data["password_m"]
        if len(password.replace(' ', '')) < 8:
            raise forms.ValidationError(
                 self.error_messages['length_error'],
                 code='password_m',
...
class UserInfoSave(forms.Form):
    ...
    # 性别
    genders_choices = UserProfile.GENDERS.items()
    # 学历
    degree_choices = UserProfile.DEGREES.items()
    provinces_choices = []
    ...
    #个人目标
    studygoals_choices = []
    studygoals = StudyGoal.objects.all()
    for study in studygoals:
        studygoals_choices.append([study.id, study.name])
    #个人基础
    studybase_choices = []
    studybases = StudyBase.objects.all()
    for study in studybases:
        studybase_choices.append([study.id, study.name])
    uid= forms.CharField(widget=forms.HiddenInput())
    nick_name = forms.CharField(widget=forms.TextInput(attrs={"class": "form-control"}),
                                max_length=10, error_messages = {"required":"昵称不能为空","max_length":"昵称不能超过10位"})
    real_name = forms.CharField(widget=forms.TextInput(attrs={"class": "form-control"}),
                                max_length=5, required = False, error_messages = {"max_length":"真实姓名不能超过5位"})
    gender = forms.ChoiceField(choices=genders_choices, required=False, widget=forms.RadioSelect())
    degree = forms.ChoiceField(choices=degree_choices, required=True, widget=forms.Select(attrs={"class": "form-control form-select-val-else"}))
    ...
    address = forms.CharField(widget=forms.TextInput(attrs={"class": "form-control", "placeholder": "请填写详细地址方便寄送毕业证等"}),
                              max_length=200, required = False, error_messages = {"max_length":"地址不能超过200位"})
    description = forms.CharField(widget=forms.Textarea(attrs={"class": "form-control",'cols':60,'rows':8,"placeholder": "可以详细描述一下您的基础和学习目标哦~"}),
    ...
    email = forms.CharField(widget=forms.TextInput(attrs={"class": "form-control"}),
                            max_length=50, required=False)
    studygoal = forms.ChoiceField(choices = (studygoals_choices), required = False,widget=forms.Select(attrs={"class": "form-control form-select-val-else","id":"aaa"}))
    studybase = forms.ChoiceField(choices = (studybase_choices), required = False, widget=forms.Select(attrs={"class": "form-control form-select-val-else","id":"bbb"}))
...
class WapMobileRegisterForm(MobileRegisterForm):
    """
    @brief 用于wap端手机注册的form.
    @note 该Form类的实现是为了最大限度的重用MobileRegisterForm类
    """
    ...
    error_messages = dict(
        duplicate_mobile="该账号已被注册",
        invaild="请输入正确手机号码",
        nonmatch_mobile_code="请输入正确的手机验证码",
        overdue_mobile_code="手机验证码已过期，请重试",
        length_error="请输入正确的密码",
    )

    # 重写错误信息, 符合WAP端的文案
    mobile = forms.CharField(max_length=11,
                             min_length=11,
                             error_messages=dict(required="请输入正确手机号码",
                                                 min_length="请输入正确手机号码",
                                                 max_length="请输入正确手机号码"))

    mobile_code = forms.CharField(max_length=6,
                                  min_length=6,
                                  error_messages=dict(required="请输入正确的手机验证码",
                                                      min_length="请输入正确的手机验证码",
                                                      max_length="请输入正确的手机验证码"))

    password_m = forms.CharField(min_length=8,
                                 max_length=20,
                                 error_messages=dict(required="请输入正确的密码",
                                                     min_length="请输入正确的密码",
                                                     max_length="请输入正确的密码"))

    def clean_password_m(self):
        """
        @brief 验证密码. 第一步验证随机码时, 跳过此步验证.
        :return:
        """
        password = self.cleaned_data["password_m"]
        if password == '-1111111':
...
class WapFindPasswordForm(FindPasswordForm):
    """
    @brief 用于wap端手机找回密码的form.
    @note 该Form类的实现是为了最大限度的重用FindPasswordForm类
    """
    # 重写错误信息, 符合WAP端的文案
    error_messages = dict(invaild="请输入正确的手机号码",
                          non_exist="该账号尚未注册")
    account = forms.CharField(max_length=11,
                              min_length=11,
                              error_messages=dict(required="请输入正确的手机号码",
                                                  min_length="请输入正的确手机号码",
                                                  max_length="请输入正的确手机号码"))

class WapFindPasswordMobileForm(FindPasswordMobileForm):
    """
    @brief 重写 FindPasswordMobileForm 的错误信息
    """
    error_messages = dict(nonmatch_mobile_code="请输入正确的手机验证码",
                          overdue_mobile_code="手机验证码已过期，请重试")

    mobile_code_f = forms.CharField(max_length=6,
                                    min_length=6,
                                    error_messages=dict(required="请输入正确的手机验证码",
                                                        min_length="请输入正的确手机验证码",
                                                        max_length="请输入正的确手机验证码"))