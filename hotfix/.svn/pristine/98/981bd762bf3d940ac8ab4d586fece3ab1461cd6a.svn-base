    ...
    try:
        ...
        if not template_vars:
            if careerId is not None:
                ...
                if len(stages) > 0:
                    courses = Course.objects.filter(stages_m=stages[0]).order_by("index", "id")
                    if len(courses) > 0:
        ...
        if careerId is not None:
            # 获取最近看的视频对应的课程
            recent_learned_lesson = UserLearningLesson.objects.filter(user=user, lesson__course__stages_m__career_course=career_course).order_by("-date_learning")
            if len(recent_learned_lesson) > 0:
                template_vars['last_course_id'] = recent_learned_lesson[0].lesson.course.id
...
@csrf_exempt
@response_init
def academicLogin(request):
    username = request.REQUEST.get('userName')
    password = request.REQUEST.get('password')
    academic_id = request.REQUEST.get('academic_id', '')
    ...
    try:
        ...
        if academic_id:
            user.academics.add(academic_id)
...
@csrf_exempt
@response_init
def academicActiveEmail(request):
    uuid = request.REQUEST.get('uuid')
    users = UserProfile.objects.filter(uuid=uuid)
    if users:
        user = users[0]
        user.valid_email = 1
        user.is_active = True
...
def academicRegPhone(request):
    ...
    try:
        ...
        if 1 == step:
            ...
            #发送验证码到手机
            send_sms_code(mobile, request.META.get('HTTP_X_REAL_IP') or request.META['REMOTE_ADDR'], 0)
...
def send_sms_code(mobile, ip, code_type):
    ...
    if MobileVerifyRecord.objects.filter(created__gt=start, mobile=mobile).count() >= settings.SMS_MAX_COUNT_MOBILE:
        raise APIException('你手机号今天验证太多了哦，请明天再试吧')
    ...
    if MobileVerifyRecord.objects.filter(Q(ip=ip),Q(created__gt = one_min_ago)).exists():
        raise APIException("距离上一次发送验证码未超过60s")
    ...
    try:
        result = tpl_send_sms.delay(apikey, tpl_id, tpl_value, mobile)
        if result['code'] != 0:
...
def regEmail(request):
    ...
    try:
        ...
        #发送注册激活邮件
        send_email_code(username, request.META.get('HTTP_X_REAL_IP') or request.META['REMOTE_ADDR'], 0)
...
def forgetPwd(request):
    ...
    try:
        ...
        if re.compile(settings.REGEX_MOBILE).match(identifier):
            send_sms_code(identifier, request.META.get('HTTP_X_REAL_IP') or request.META['REMOTE_ADDR'], 1)
            Response.message = '请查收验证码'
            Response.data['type'] = RESET_PASSWORD_BY_MOBILE

        elif re.compile(settings.REGEX_EMAIL).match(identifier):
            send_email_code(identifier, request.META.get('HTTP_X_REAL_IP') or request.META['REMOTE_ADDR'], 1)
...
def getMyClass(request):
    try:
        ...
        classing = Class.objects.filter(teachers = user,status = 1,is_active = True).annotate(num_students=Count('students'))
        Response.data = {
            "list": [
                {
                    "class_id": str(c.id),
...
def getStudentDetail(request):
    try:
        ...
        if user not in class_.teachers.all():
...
def _getCourseProgress(student_id, course_id, Response):
    try:
        ...
        homework_list = Homework.objects.filter(examine_type=1, relation_type=1,
                                                relation_id__in=course.lesson_set.filter(have_homework=True).values_list("id"))
...
def setCoursePlayProgress(request):
    ...
    stage_id = request.REQUEST.get('stage_id') or None
    ...
    try:
        ...
        if not learning.is_complete:
            learning.is_complete=True
            learning.save()
            update_study_point_score(student=user, study_point=1, course=learning.lesson.course, rebuild_count=0, lesson_id = lesson_id, update_type=1)
        ...
        if stage_id is not None:
            try:
                career_course = Stage.objects.get(pk = stage_id).career_course # cur_lesson.course.stages.career_course
            except Exception as e:
                pass
...
def getCommentList(request):
    ...
    page = request.REQUEST.get('page',1)
    pageSize = request.REQUEST.get('pageSize',15)
    child_comment=[]
    cur_lesson = None
    try:
    ...
    if discuzz_provider == 'FPS':
        ...
        _url = _host + "course_discuss/list/"
        _params = dict(lesson_id=lessonId, page=page, pageSize=pageSize)
        _text = ''
        try:
            _text = requests.post(_url, _params).text
        except Exception as e:
            print e
            Response.message = e.message
            Response.success = False
        ...
        Response.message = '获取成功'
        Response.success = True
        discuss_list = json.loads(_text)
        Response.data = {
            "list":[{
                "comment_id":p['id'],
                "user_id":p['user'],
                "user_name":UserProfile.objects.get(pk=p['user']).nick_name,
                "avatar":settings.SITE_URL+settings.MEDIA_URL+str(UserProfile.objects.get(pk=p['user']).avatar_url),
                "desc":p['content'],
                "date":str(p['date_publish']),
                "child_list":[{
                    "comment_id":c['id'],
                    "user_id":c['user'],
                    "user_name":UserProfile.objects.get(pk=c['user']).nick_name,
                    "avatar":settings.SITE_URL+settings.MEDIA_URL+str(UserProfile.objects.get(pk=c['user']).avatar_url),
                    "desc":c['content'],
                    "date":str(c['date_publish'])
                } for c in p['child_list'] ]
            } for p in discuss_list]
        }
    return HttpResponse(Response.dumps(), content_type="application/json")
...
def sendComment(request):
    user = checkUser(request, 'all')
    lessonId = request.REQUEST.get('lessonId')
    parentId = request.REQUEST.get('commentId', 0)
    ...
    if discuzz_provider == 'FPS':
        # 使用FPS接口发送评论
        _host = settings.FPS_HOST
        _url = _host + "course_discuss/comment/"
        _params = dict(comment=desc, parentId=parentId, lesson_id=lessonId, user_id=user.id)
        try:
    ...
    if parentId is not None:
        try:
            ...
            if discuzz_provider == 'FPS':
                _host = settings.FPS_HOST
                _url = _host + "course_discuss/get_comment_user/"
                _params = dict(comment_id=parentId)
                r = requests.post(_url, _params)
                if r.status_code == 200:
                    ret = json.loads(r.text)
                    if ret['message'] == 'success':
                        puser = UserProfile.objects.get(pk=int(ret['user']))
        ...
        try:
            ...
            for stage in lesson.course.stages_m.all():
                ts = ts + [teachers['teachers'] for teachers in Class.objects.filter(career_course=stage.career_course).values("teachers")]
...
def getMyCourseStage(request):
    ...
    try:
        ...
        for s in stage_set:
            ...
            course_set= Course.objects.filter(stages_m = s, is_active=True)
            scores = {}
            for c in course_set:
                try:
...
def getMyHomeworkList(request):
    ...
    try:
        ...
        works=[]
        for lesson in lessons:
            if hasattr(lesson, "have_homework"):
                if not lesson.have_homework:
                    continue
...
def _answer_quiz(request, user, course, paper):
    ...
    if len(get_uncomplete_quiz(user, paper, rebuild_count)) == 0:
        ...
        update_study_point_score(user, paper_study_point, paper_score, paper, paper_record, None, None,rebuild_count)
...
def checkUpdate(request):
    ...
    try:
        ...
        #最新版本号
        newVersion_list = AndroidVersion.objects.filter(type=AndroidVersion.TYPE_STUDENT).order_by("-id")
    ...
    try:
        ...
        if cur_careercourse.buybtn_status == 0 :
            if payType == "0":
                ...
                # 全额
                # setattr(cur_careercourse, "total_price", cur_careercourse.net_price)
                ...
                # 试学首付
                # setattr(cur_careercourse, "first_payment", cur_careercourse.try_price)
        ...
        if cur_careercourse.buybtn_status == 0 and payType == "1":
            purchase.final_payment_price = cur_careercourse.contract_price - cur_careercourse.try_price
        purchase.save()
...
@csrf_exempt
@response_init
def getRTInfo(request):
    careerId = request.REQUEST.get('careerId', None) #职业课程ID

    try:
        if careerId is None:
            raise APIException("职业课程ID不能为空哦")
        ...
        try:
            ...
            raise APIException('没有对应的职业课程')

        # 根据职业课程和用户找到用户对应的班级
        Class.objects.filter(career_course=career_course)
        class_students = ClassStudents.objects.filter(user=user,
                                                     student_class__in=Class.objects.filter(career_course=career_course))
        if len(class_students) > 0:
            class_students = class_students[0]
        else:
            raise APIException('你还未加入该班级')

        # 获取直播室信息
        try:
            live_room = LiveRoom.objects.get(live_class=class_students.student_class)
            Response.data['state'] = live_room.live_is_open
            Response.data['domain'] = 'maiziedu.gensee.com'
            Response.data['room_num'] = live_room.live_code
            Response.data['join_pwd'] = live_room.student_token
            Response.data['join_pwd_client'] = live_room.student_client_token
            Response.data['SDK_ID'] = live_room.live_id
        except LiveRoom.DoesNotExist:
            Response.data['state'] = '0'
            Response.data['domain'] = ''
            Response.data['room_num'] = ''
            Response.data['join_pwd'] = ''
            Response.data['join_pwd_client'] = ''
            Response.data['SDK_ID'] = ''
        Response.message = '数据获取成功'
        Response.success = True
