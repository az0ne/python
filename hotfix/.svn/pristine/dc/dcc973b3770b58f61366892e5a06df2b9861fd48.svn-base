define(function (require, exports, module) {
    ...
    var login_popup = require('./main').login_popup,
        //测试用,不要开
        //Videojs = require('videojs').Videojs,
        startPause = require('videojs2').startPause,
        playerWrap = $('video'),
        player = playerWrap[0],
        headTitle1,
        headTitle2,
    ...
    var init = function () {
        ...
        //var _isLogin = $('#this_user').val(); // 是否登录
        function is_login() {
            return $('#this_user').val() == 'True';
        }
        ...
        //　是否具有职业课程
        function if_has_career_course() {
            return $('#if_have_career_course').val();
        }
        ...
        var video_current_time = $.cookie('lesson_' + $('#lesson_id').val());
        ...
        playerWrap.ready(function () {
            ...
            playerWrap.on("play", function () {
                ...
                if (!is_login()) {
                ...
                } else if (get_current_lesson_index() == 3 && need_pay() && !is_paid() && if_has_career_course()) { // 判断当前播放的是第几章
                    ...
                    return false;
                } else if (get_current_lesson_index() > 3 && need_pay() && !is_paid() && if_has_career_course()) {
                    if (zy_nbo && zy_nbo2) {
                        zy_nbo2 = false;
                        //测试用,不要开
                        //player.pause();
        ...
        function getPlayPostion() {
            ...
            $.cookie('lesson_' + $('#lesson_id').val(), current_position);
            if (current_position) {
                if ($('#this_user').val() == "True") {
                    //判断视频是否播放到整体进度的95%
                    var video_total_time = $('#video_length').val();
                    // 如果是在lps3里学习,并且视频播放时间到了规定的百分比,记录学习记录
                    if (to_be_update == true && $('#class_id').val().length > 0 && current_position / video_total_time > $('#video_exam_complete').val()) {
                        // 更新是否完成该章节的学习
                        to_be_update = false;
                        $.get("/lps3/student/ajax/item_lesson/update/" + $('#user_item').val());
                    }
                }
            }
        }
        ...
        tiwenBtn.on('click', function (event) {
            if(!is_login()){
                ...
                } else if($('#group_name').val() == 'teacher'){
        ...
        // 原图查看弹窗
        imgpopup = require('./main').imgpopup;
        var _src = null;
                        ...
                        $.ajax({
                            ...
                            success: function (result) {
                                if (result['success']) {
                                    ...
                                    //var praise_count = parseInt(small.html());

                                    //// 点赞成功
                                    //if (action == 'mark') {
                                    //    small.html(praise_count + 1);
                                    //    ths.addClass("jia-good");
                                    //} else if (action == 'cancel') {
                                    //    small.html(praise_count - 1);
                                    //    ths.removeClass("jia-good");
                                    //}
