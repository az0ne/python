...
from db.api.homepage import careerlink as api_careerlink
...
def career_link(request):
    ...
    if careerlink.is_error():
        return render_to_response("404.html", {}, context_instance=RequestContext(request))
    careerlinks = careerlink.result()["result"]
    for e in careerlinks:
        e['CareerType_display'] = CareerType.get(e['type'], 'N/A')
    c = {"careerlinks": careerlinks,
         "page": {"page_index": page_index, "rows_count": careerlink.result()["rows_count"],
                  "page_size": settings.PAGE_SIZE,
                  "page_count": careerlink.result()["page_count"]}}
    return render_to_response("mz_seo/mz_career_link.html", c, context_instance=RequestContext(request))
...
def career_link_save(request):
    ...
    type = tool.get_param_by_request(request.POST, 'type', "", str)
    if action == 'add':
        careerlink = api_careerlink.insert_careerlink(title,url,index,career_id,type)
        if careerlink.is_error():
            return render_to_response("404.html", {}, context_instance=RequestContext(request))
    elif action == 'edit':
        careerlink = api_careerlink.updatecareerlink(_id,title,url,index,career_id,type)
        if careerlink.is_error():
            return render_to_response("404.html", {}, context_instance=RequestContext(request))
    return HttpResponseRedirect("/seo/mz_career_link/list/?action=query&page_index=" + str(page_index))
...
def career_link_edit(request):
    ...
    return render_to_response("mz_seo/mz_career_link_add.html", c, context_instance=RequestContext(request))