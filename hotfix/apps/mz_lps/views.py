...
def learning_plan_view(request, careercourse_id):
    ...
    :return:
    '''
    return render(request, 'mz_common/failure.html',{'reason':'此页面已过期。<a href="'+str(settings.SITE_URL)+'">返回首页</a>'})
    ################# 个人中心 -- 左边 开始  ########################
    user = request.user
    ...
    if stage_id == None :
        ...
        recent_learned_lesson = UserLearningLesson.objects.filter(user=user, lesson__course__in = courselist).order_by("-date_learning")
        ...
        if recent_learned_lesson:
            ...
            stagel = recent_learned_lesson[0].lesson.course.stages_m.all()
            cur_stage = stagel[0]
    ...
    if cur_careercourse.is_unlockstage:
        ...
        for stage in cur_careercourse.stage_set.all():
            ...
            stage.courses_list = Course.objects.filter(stages_m=stage).order_by("index", "id")
...
def get_all_course_novice_info(request, careercourse_id):
    try:
        ...
        return HttpResponse('{"status":"failure","message":"不存在该职业课程"}', content_type="application/json")
    courses = Course.objects.filter(stages_m__career_course=cur_careercourse).order_by("index", "id")
...
def build_learning_plan(request, careercourse_id):
    ...
    try:
        ...
        if len(planning) == 0:
            ...
            planning = planning[0]
        all_course_list = Course.objects.filter(stages_m__career_course=cur_careercourse)
        # 获取用户提交的选择数据
...
def project_upload(request):
    ...
    try:
        ...
        if UserUnlockStage.objects.filter(user=request.user, stage__in = cur_course.stages_m.all()).count() == 0:
    ...
    if files:
        ...
        if result[0] == True:
            ...
            rebuild_count = get_rebuild_count(request.user, course_id)  # 重修次数
            try:
                ...
                try:
                    ...
                    project_record.save()
                    # 首次上传时加10学力
                    update_study_point_score(request.user, 10, examine=project, examine_record=project_record, update_type=3)
                except Exception as e:
                    logger.error(e)
            except Exception as e:
                logger.error(e)
            ...
            try:
                ...
                for stage in cur_course.stages_m.all():
                    stage_id = stage.id
                    class_student = ClassStudents.objects.get(user=request.user, student_class__career_course=Stage.objects.get(pk=stage_id).career_course)
                    # 获取该职业课程老师班级下的所有学员
                    alert_msg = str(request.user.nick_name)+"已上传了"+str(cur_course.name)+\
                                "课程的项目制作，<a href='http://www.maiziedu.com/lps2/teach/plan/"+str(class_student.student_class.id)+\
...
def answer_quiz(request, quiz_id, select):
    ...
    if len(get_uncomplete_quiz(user, cur_paper, rebuild_count)) == 0:
        ...
        update_study_point_score(user, paper_study_point, paper_score, cur_paper, paper_record, None, None, rebuild_count)
