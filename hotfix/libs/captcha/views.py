...
def verify_code(request,hash_key,code):
    try:
        CaptchaStore.objects.get(response=code, hashkey=hash_key, expiration__gt=get_safe_now())
        return HttpResponse(json.dumps({"status":'success',"msg":''}), content_type="application/json")
    except CaptchaStore.DoesNotExist:
        return HttpResponse(json.dumps({"status":'fail',"msg":''}), content_type="application/json")
