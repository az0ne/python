...
var nowtimes, nowtimeselect, is_click_experedit = false, is_click_educedit = false,experliId,educliId;
nowtimes = new Date();
...
var init = function(){
	...
	$('.resume .info .submitBtn input').on('click',function(){
		var tips = $('.resume .info .save-tips');
		if(name.val().length == 0){
			name.siblings('.error').text('请输入您的姓名');
			return;
	...
	experlist.on({'click':function(){
		...
		if(is_click_experedit == true){
			th.siblings('li').children('.create').remove();
			th.siblings('li').children('div').fadeIn(200);
		}
		...
		experEndTime = exper.find('#experEndTime').val();
		experStartTime = exper.find('#experStartTime').val();
	...
	educlist.on({'click':function(){
		...
		educStartTime = educ.find('#educStartTime').val();
		educEndTime = educ.find('#educEndTime').val();
		educSelect();
...
function personalInfoSelect(){
	//出生日期
	var myTimer = birthtime||"1970-01-01";
...
function experSelect(){
	//任职时间
	var myexperStartTimer = experStartTime||"1990-01-01";
	...
	var myexperEndTimer = experEndTime||"1990-01-03";
 ...
 */
function educSelect(){
	var myeducStartTimer = educStartTime||"1990-01-01";
	...
	var myeducEndTimer = educEndTime||"1990-01-03";
