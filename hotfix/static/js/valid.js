...
(function($) {
	$.fn.valid = function(iSet) {
		...
		this.each(function() {
			var th=$(this);
			$(this).find("input").filter('input:text, input:password').each(function(index, element) {
				$(this).blur(function(){
						...
						if($(this).attr("valid").indexOf('num')>-1&&!$(this).hasClass("validerror")){
						...
						if($(this).attr("valid").indexOf('phone')>-1&&!$(this).hasClass("validerror")){
			...
			$(this).find("input").filter('input:submit').each(function(index, element) {				
				th.submit(function(){
					...
					th.find("input").filter('input:text, input:password').trigger("blur");
					th.find("select").trigger("blur");
					
					th.find("input").filter('input:text, input:password').each(function(index, element) {
		...
		function verr(th,str){
			...
			if(!(th.parent().find("span.verror").length>0)){
				th.parent().append("<span class='verror'>"+str+"</span>");
			}
			else{
				th.parent().find("span.verror").remove();
				th.parent().find("span.vright").remove();
				th.parent().append("<span class='verror'>"+str+"</span>");
			}
		}
		function vrig(th){
			th.removeClass("validerror");
			if(!(th.parent().find("span.vright").length>0)){				
		...
		function checkMobile( s ){  
			var regu =/^(0|86|17951)?(13[0-9]|15[012356789]|17[678]|18[0-9]|14[57])[0-9]{8}$/g;
			var re = new RegExp(regu);
			if (re.test(s.trim())) {
				return true;
			}else{
